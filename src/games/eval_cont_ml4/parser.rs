...
mod tests {
    ...
    #[test]
    fn rejects_unparenthesized_negative_int_in_eval_arg_frame() {
        let source = "1 => {|- _ -2} evalto 1 by C-Ret {}";
        let err = parse_source(source).expect_err("parse should fail");
        assert_eq!(err.kind(), CheckErrorKind::Parse);
        assert!(err
            .message()
    ...
    fn parses_application_tighter_than_cons() {
        ...
        let EvalContML4Judgment::EvalTo { expr, .. } = parsed.judgment else {
            panic!("expected evalto");
        };
        assert_eq!(
            expr,
            EvalContML4Expr::Cons {

...
mod tests {
    ...
    #[test]
    fn tokenizes_continuation_syntax() {
        let source = "|- letcc k in 1 + k 2 >> { 3 + _ } evalto 5 by E-LetCc {}";
        let tokens = tokenize(source).expect("tokenize should succeed");
        let kinds = tokens
            .into_iter()
            .map(|token| token.kind)
            .collect::<Vec<_>>();
        assert!(kinds.contains(&TokenKind::LetCc));
        assert!(kinds.contains(&TokenKind::ContArrow));
        assert!(kinds.contains(&TokenKind::Underscore));
    }

    #[test]
    fn tokenizes_continuation_apply_arrow() {
        let source = "2 => {3 + _} >> _ evalto 5 by C-Plus {}";
        let tokens = tokenize(source).expect("tokenize should succeed");
        let kinds = tokens
            .into_iter()
            .map(|token| token.kind)

...
mod tests {
    ...
    fn tokenizes_simple_derivation() {
        let source = "x = 3 |- x evalto 3 by E-Var1 {}";
        ...
        assert_eq!(tokens[5].kind, TokenKind::EvalTo);
        assert_eq!(tokens[6].kind, TokenKind::Int(3));
        assert_eq!(tokens[7].kind, TokenKind::By);
        assert_eq!(tokens[8].kind, TokenKind::Identifier("E-Var1".to_string()));
    }
    ...
    fn reports_unexpected_character() {
        let source = "@";
        let err = tokenize(source).expect_err("tokenize should fail");
        assert!(err.to_string().contains("unexpected character"));
        let span = err.span().expect("parse error should have span");

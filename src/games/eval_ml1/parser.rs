...
pub fn parse_source(source: &str) -> Result<EvalML1Derivation, CheckError> {
...
pub(super) fn parse_judgment_source(source: &str) -> Result<EvalML1Judgment, CheckError> {
    if source.trim().is_empty() {
        return Err(CheckError::parse("input is empty"));
    }
...
impl Parser {
    ...
    fn parse_derivation(&mut self) -> Result<EvalML1Derivation, CheckError> {
    ...
    fn parse_judgment(&mut self) -> Result<EvalML1Judgment, CheckError> {
        let expr = self.parse_expr()?;
        if self.consume_evalto() {
            let value = self.parse_value()?;
            return Ok(EvalML1Judgment::EvalTo { expr, value });
        }

        if let EvalML1Expr::Int(left) = expr {
            if self.consume_plus_word() {
                let right = self.parse_int_literal()?;
                self.expect_keyword_is()?;
                let result = self.parse_int_literal()?;
                return Ok(EvalML1Judgment::PlusIs {
                    left,
                    right,
                    result,
                });
            ...
            if self.consume_minus_word() {
                ...
                self.expect_keyword_is()?;
                let result = self.parse_int_literal()?;
                return Ok(EvalML1Judgment::MinusIs {
                    left,
                    right,
                    result,
                });
            ...
            if self.consume_times_word() {
                ...
                self.expect_keyword_is()?;
                let result = self.parse_int_literal()?;
                return Ok(EvalML1Judgment::TimesIs {
                    left,
                    right,
                    result,
                });
            ...
            if self.consume_less() {
                ...
                return Ok(EvalML1Judgment::LessThanIs {
                    left,
                    right,
                    result,
                });
    ...
    fn parse_expr(&mut self) -> Result<EvalML1Expr, CheckError> {
        self.parse_if_expr()
    }

    fn parse_if_expr(&mut self) -> Result<EvalML1Expr, CheckError> {
    ...
    fn parse_lt_expr(&mut self) -> Result<EvalML1Expr, CheckError> {
    ...
    fn parse_add_expr(&mut self) -> Result<EvalML1Expr, CheckError> {
    ...
    fn parse_mul_expr(&mut self) -> Result<EvalML1Expr, CheckError> {
    ...
    fn parse_atom_expr(&mut self) -> Result<EvalML1Expr, CheckError> {
        ...
        if let Some(value) = self.consume_int_literal() {
            return Ok(EvalML1Expr::Int(value));
        }

        if let Some(value) = self.consume_bool_literal() {
            return Ok(EvalML1Expr::Bool(value));
        }
    ...
    fn parse_value(&mut self) -> Result<EvalML1Value, CheckError> {
        if let Some(value) = self.consume_int_literal() {
            return Ok(EvalML1Value::Int(value));
        }
        if let Some(value) = self.consume_bool_literal() {
            return Ok(EvalML1Value::Bool(value));
        }
    ...
    fn parse_subderivations(&mut self) -> Result<Vec<EvalML1Derivation>, CheckError> {
        if self.at_rbrace() {
            return Ok(Vec::new());
        }
...
mod tests {
    ...
    #[test]
    fn parses_fixture_025() {
        let source = include_str!("../../../copl/025.copl");
    ...
    #[test]
    fn parses_fixtures_026_to_030() {
        for source in [
            include_str!("../../../copl/026.copl"),

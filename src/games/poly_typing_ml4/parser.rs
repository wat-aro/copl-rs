...
pub fn parse_source(source: &str) -> Result<PolyTypingML4Derivation, CheckError> {
...
pub(super) fn parse_judgment_source(source: &str) -> Result<PolyTypingML4Judgment, CheckError> {
    if source.trim().is_empty() {
        return Err(CheckError::parse("input is empty"));
    }
...
impl Parser {
    ...
    fn parse_derivation(&mut self) -> Result<PolyTypingML4Derivation, CheckError> {
    ...
    fn parse_judgment(&mut self) -> Result<PolyTypingML4Judgment, CheckError> {
        ...
        Ok(PolyTypingML4Judgment::HasType { env, expr, ty })
    }
    ...
    fn parse_env(&mut self) -> Result<PolyTypingML4Env, CheckError> {
        if self.consume_turnstile() {
            return Ok(PolyTypingML4Env::default());
        }
        ...
        Ok(PolyTypingML4Env(bindings))
    }

    fn parse_binding(&mut self) -> Result<PolyTypingML4Binding, CheckError> {
        let name = self.parse_identifier()?;
        self.expect_colon()?;
        let scheme = self.parse_scheme()?;
        Ok(PolyTypingML4Binding { name, scheme })
    }

    fn parse_scheme(&mut self) -> Result<PolyTypingML4TypeScheme, CheckError> {
        let start_index = self.index;
        let mut quantified = Vec::new();

        while let Some(name) = self.consume_type_var_name() {
            quantified.push(name);
        }

        if !quantified.is_empty() && self.consume_dot() {
            let ty = self.parse_type()?;
            return Ok(PolyTypingML4TypeScheme { quantified, ty });
        }

        self.index = start_index;
        let ty = self.parse_type()?;
        Ok(PolyTypingML4TypeScheme::mono(ty))
    }

    fn parse_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        self.parse_let_expr()
    }

    fn parse_let_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        if self.consume_keyword_let() {
            if self.consume_keyword_rec() {
                ...
                return Ok(PolyTypingML4Expr::LetRec {
                    name,
                    param,
            ...
            self.expect_keyword_in()?;
            let body = self.parse_expr()?;
            return Ok(PolyTypingML4Expr::Let {
                name,
    ...
    fn parse_if_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        if self.consume_keyword_if() {
            ...
            return Ok(PolyTypingML4Expr::If {
    ...
    fn parse_fun_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        if self.consume_keyword_fun() {
            let param = self.parse_identifier()?;
            self.expect_arrow()?;
            let body = self.parse_expr()?;
            return Ok(PolyTypingML4Expr::Fun {
                param,
                body: Box::new(body),
            });
        }
    ...
    fn parse_match_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        if self.consume_keyword_match() {
            ...
            return Ok(PolyTypingML4Expr::Match {
    ...
    fn parse_cons_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        let head = self.parse_lt_expr()?;
        if self.consume_cons_symbol() {
            let tail = self.parse_cons_expr()?;
            Ok(PolyTypingML4Expr::Cons {
                head: Box::new(head),
                tail: Box::new(tail),
            })
        } else {
    ...
    fn parse_lt_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
    ...
    fn parse_add_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
    ...
    fn parse_app_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
    ...
    fn parse_atom_expr(&mut self) -> Result<PolyTypingML4Expr, CheckError> {
        ...
        if self.consume_lbracket() {
            self.expect_rbracket()?;
            return Ok(PolyTypingML4Expr::Nil);
        }

        if let Some(value) = self.consume_int_literal() {
            return Ok(PolyTypingML4Expr::Int(value));
        }

        if let Some(value) = self.consume_bool_literal() {
            return Ok(PolyTypingML4Expr::Bool(value));
        }

        if let Some(name) = self.consume_identifier() {
            return Ok(PolyTypingML4Expr::Var(name));
        }
    ...
    fn parse_type(&mut self) -> Result<PolyTypingML4Type, CheckError> {
        self.parse_fun_type()
    }

    fn parse_fun_type(&mut self) -> Result<PolyTypingML4Type, CheckError> {
        let param = self.parse_list_type()?;
        if self.consume_arrow() {
            let ret = self.parse_fun_type()?;
            Ok(PolyTypingML4Type::Fun {
                param: Box::new(param),
                ret: Box::new(ret),
            })
        } else {
    ...
    fn parse_list_type(&mut self) -> Result<PolyTypingML4Type, CheckError> {
        let mut ty = self.parse_atomic_type()?;
        while self.consume_keyword_list() {
            ty = PolyTypingML4Type::List(Box::new(ty));
        }
        Ok(ty)
    }

    fn parse_atomic_type(&mut self) -> Result<PolyTypingML4Type, CheckError> {
        if self.consume_lparen() {
            let ty = self.parse_type()?;
            self.expect_rparen()?;
            return Ok(ty);
        ...
        if self.consume_keyword_int_type() {
            return Ok(PolyTypingML4Type::Int);
        }

        if self.consume_keyword_bool_type() {
            return Ok(PolyTypingML4Type::Bool);
        }

        if let Some(name) = self.consume_type_var_name() {
            return Ok(PolyTypingML4Type::Var(name));
        }
    ...
    fn parse_subderivations(&mut self) -> Result<Vec<PolyTypingML4Derivation>, CheckError> {
        if self.at_rbrace() {
            return Ok(Vec::new());
        }
    ...
    fn consume_type_var_name(&mut self) -> Option<String> {
        let TokenKind::TypeVar(name) = &self.peek().kind else {
            return None;
        };
        let name = name.clone();
        self.bump();
...
mod tests {
    ...
    use crate::games::poly_typing_ml4::syntax::{
        PolyTypingML4BinOp, PolyTypingML4Env, PolyTypingML4Expr, PolyTypingML4Judgment,
        PolyTypingML4Type,
    };

    #[test]
    fn parses_fixture_107() {
        let source = include_str!("../../../copl/107.copl");
        let parsed = parse_source(source).expect("fixture should parse");
        assert_eq!(parsed.rule_name, "T-Abs");
        assert_eq!(parsed.subderivations.len(), 1);
        assert_eq!(
            parsed.judgment,
            PolyTypingML4Judgment::HasType {
                env: PolyTypingML4Env::default(),
                expr: PolyTypingML4Expr::Fun {
                    param: "x".to_string(),
                    body: Box::new(PolyTypingML4Expr::Var("x".to_string())),
                },
...
"#;
    ...
    fn parses_match_expression_shape() {
        ...
        let parsed = parse_source(source).expect("parser should succeed");
        let PolyTypingML4Judgment::HasType { expr, ty, .. } = parsed.judgment;
        assert_eq!(ty, PolyTypingML4Type::Int);
        assert_eq!(
            expr,
            PolyTypingML4Expr::Match {
                scrutinee: Box::new(PolyTypingML4Expr::Var("x".to_string())),
                nil_case: Box::new(PolyTypingML4Expr::Int(0)),
                head_name: "a".to_string(),
                tail_name: "b".to_string(),
                cons_case: Box::new(PolyTypingML4Expr::Var("a".to_string())),
            }
        );
    }

    #[test]
    fn parses_quantified_type_scheme_in_environment() {
        let source = "f : 'a 'b.'a -> 'b -> 'a |- f : int -> bool -> int by T-Var {}";
        let parsed = parse_source(source).expect("parser should succeed");
        let PolyTypingML4Judgment::HasType { env, .. } = parsed.judgment;
        assert_eq!(env.0.len(), 1);
        assert_eq!(env.0[0].name, "f");
        assert_eq!(env.0[0].scheme.to_string(), "'a 'b.'a -> 'b -> 'a");
    }

    #[test]
    fn preserves_operator_precedence() {
        let source = "|- 1 + 2 * 3 : int by T-Unknown {}";
        let parsed = parse_source(source).expect("parser should succeed");
        let PolyTypingML4Judgment::HasType { expr, .. } = parsed.judgment;
        assert_eq!(
            expr,
            PolyTypingML4Expr::BinOp {
                op: PolyTypingML4BinOp::Plus,
                left: Box::new(PolyTypingML4Expr::Int(1)),
                right: Box::new(PolyTypingML4Expr::BinOp {
                    op: PolyTypingML4BinOp::Times,
                    left: Box::new(PolyTypingML4Expr::Int(2)),
                    right: Box::new(PolyTypingML4Expr::Int(3)),
                }),
            }
        );
    }
    ...
    fn parses_application_tighter_than_cons() {
        ...
        assert_eq!(
            expr,
            PolyTypingML4Expr::Cons {
                head: Box::new(PolyTypingML4Expr::App {
                    func: Box::new(PolyTypingML4Expr::Var("f".to_string())),
                    arg: Box::new(PolyTypingML4Expr::Int(1)),
                }),
                tail: Box::new(PolyTypingML4Expr::Cons {
                    head: Box::new(PolyTypingML4Expr::Int(2)),
                    tail: Box::new(PolyTypingML4Expr::Nil),
                }),
            }
        );
    }
    ...
    fn parses_judgment_only_input_for_prover() {
        let parsed =
            parse_judgment_source("|- fun x -> x : 'a -> 'a").expect("judgment should parse");
        assert_eq!(
            parsed,
            PolyTypingML4Judgment::HasType {
                env: PolyTypingML4Env::default(),

...
impl<'a> Lexer<'a> {
    ...
    fn next_token(&mut self) -> Result<Token, CheckError> {
        ...
        match ch {
            ...
            '#' => {
                self.bump_char('#');
                let value = self.lex_usize_literal()?;
                Ok(Token {
    ...
    fn lex_usize_literal(&mut self) -> Result<usize, CheckError> {
        let start = self.current_span();
        let start_index = self.index;
        ...
        if !saw_digit {
            return Err(self.error("expected de Bruijn index", start));
        }

        let text = &self.source[start_index..self.index];
        text.parse::<usize>()
            .map_err(|_| self.error("invalid de Bruijn index", start))
    }
...
mod tests {
    ...
    #[test]
    fn tokenizes_translation_syntax() {
        let source = "x |- let y = x in y ==> let . = #1 in #1 by Tr-Let {}";
        ...
        assert!(kinds.contains(&TokenKind::Translates));
        assert!(kinds.contains(&TokenKind::Dot));
        assert!(kinds.contains(&TokenKind::Index(1)));
    }

    #[test]
    fn tokenizes_negative_integers() {
        let source = "|- -2 ==> -2 by Tr-Int {}";
        let tokens = tokenize(source).expect("tokenize should succeed");
    ...
    fn reports_unexpected_character() {
        let err = tokenize("|- 1 @ 2 ==> 3 by Tr-Int {}").expect_err("should fail");

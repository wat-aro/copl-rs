...
pub fn parse_source(source: &str) -> Result<CompareNat1Derivation, CheckError> {
...
pub(super) fn parse_judgment_source(source: &str) -> Result<CompareNat1Judgment, CheckError> {
    if source.trim().is_empty() {
        return Err(CheckError::parse("input is empty"));
    }
...
impl Parser {
    ...
    fn parse_derivation(&mut self) -> Result<CompareNat1Derivation, CheckError> {
    ...
    fn parse_judgment(&mut self) -> Result<CompareNat1Judgment, CheckError> {
        let left = self.parse_term()?;
        self.expect_keyword_is()?;
        self.expect_keyword_less()?;
        self.expect_keyword_than()?;
        let right = self.parse_term()?;
        Ok(CompareNat1Judgment { left, right })
    }

    fn parse_term(&mut self) -> Result<CompareNat1Term, CheckError> {
        if self.consume_z() {
            return Ok(CompareNat1Term::Z);
        }
        if self.consume_s() {
            self.expect_lparen()?;
            let inner = self.parse_term()?;
            self.expect_rparen()?;
            return Ok(CompareNat1Term::S(Box::new(inner)));
        }
    ...
    fn parse_subderivations(&mut self) -> Result<Vec<CompareNat1Derivation>, CheckError> {
        if self.at_rbrace() {
            return Ok(Vec::new());
        }
...
mod tests {
    ...
    #[test]
    fn parses_fixture_009() {
        let source = include_str!("../../../copl/009.copl");
        ...
        assert_eq!(
            parsed.judgment,
            CompareNat1Judgment {
                left: CompareNat1Term::S(Box::new(CompareNat1Term::S(Box::new(
                    CompareNat1Term::Z,
                )))),
                right: CompareNat1Term::S(Box::new(CompareNat1Term::S(Box::new(
                    CompareNat1Term::S(Box::new(CompareNat1Term::Z)),
                )))),
            }
        );
    }
    ...
    #[test]
    fn parses_fixture_012() {
        let source = include_str!("../../../copl/012.copl");
...
"#;
    ...
    fn rejects_derivation_input_in_judgment_only_parser() {
        let err = parse_judgment_source("S(Z) is less than S(S(Z)) by L-Succ {}")

...
pub fn parse_source(source: &str) -> Result<EvalML3Derivation, CheckError> {
...
pub(super) fn parse_judgment_source(source: &str) -> Result<EvalML3Judgment, CheckError> {
    if source.trim().is_empty() {
        return Err(CheckError::parse("input is empty"));
    }
...
impl Parser {
    ...
    fn parse_derivation(&mut self) -> Result<EvalML3Derivation, CheckError> {
    ...
    fn parse_judgment(&mut self) -> Result<EvalML3Judgment, CheckError> {
        ...
        if let EvalML3Expr::Int(left) = expr {
            ...
            if self.consume_minus_word() {
                ...
                self.expect_keyword_is()?;
                let result = self.parse_int_literal()?;
                return Ok(EvalML3Judgment::MinusIs {
                    left,
                    right,
                    result,
                });
            ...
            if self.consume_times_word() {
                ...
                self.expect_keyword_is()?;
                let result = self.parse_int_literal()?;
                return Ok(EvalML3Judgment::TimesIs {
                    left,
                    right,
                    result,
                });
            ...
            if self.consume_less() {
                ...
                return Ok(EvalML3Judgment::LessThanIs {
                    left,
                    right,
                    result,
                });
    ...
    fn parse_env(&mut self) -> Result<EvalML3Env, CheckError> {
        if self.consume_turnstile() {
            return Ok(EvalML3Env::default());
        }
        ...
        Ok(EvalML3Env(bindings))
    }

    fn parse_env_in_parens(&mut self) -> Result<EvalML3Env, CheckError> {
        if self.at_rparen() {
            return Ok(EvalML3Env::default());
        }
    ...
    fn parse_binding(&mut self) -> Result<EvalML3Binding, CheckError> {
        let name = self.parse_identifier()?;
        self.expect_equal()?;
        let value = self.parse_value()?;
        Ok(EvalML3Binding { name, value })
    }

    fn parse_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
        self.parse_let_expr()
    }

    fn parse_let_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
    ...
    fn parse_if_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
    ...
    fn parse_fun_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
        if self.consume_keyword_fun() {
            let param = self.parse_identifier()?;
            self.expect_arrow()?;
            let body = self.parse_expr()?;
            return Ok(EvalML3Expr::Fun {
                param,
                body: Box::new(body),
            });
        }
    ...
    fn parse_lt_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
    ...
    fn parse_add_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
    ...
    fn parse_atom_expr(&mut self) -> Result<EvalML3Expr, CheckError> {
        ...
        if let Some(value) = self.consume_int_literal() {
            return Ok(EvalML3Expr::Int(value));
        }

        if let Some(value) = self.consume_bool_literal() {
            return Ok(EvalML3Expr::Bool(value));
        }

        if let Some(name) = self.consume_identifier() {
            return Ok(EvalML3Expr::Var(name));
        }
    ...
    fn parse_value(&mut self) -> Result<EvalML3Value, CheckError> {
        if let Some(value) = self.consume_int_literal() {
            return Ok(EvalML3Value::Int(value));
        }
        if let Some(value) = self.consume_bool_literal() {
            return Ok(EvalML3Value::Bool(value));
        }
        ...
        if self.consume_lparen() {
            ...
            if self.consume_keyword_rec() {
                ...
                return Ok(EvalML3Value::RecClosure {
                    env,
                    name,
                    param,
                    body,
            ...
            let body = self.parse_expr()?;
            self.expect_rbracket()?;
            return Ok(EvalML3Value::Closure { env, param, body });
        }
    ...
    fn parse_subderivations(&mut self) -> Result<Vec<EvalML3Derivation>, CheckError> {
        if self.at_rbrace() {
            return Ok(Vec::new());
        }
...
mod tests {
    ...
    use crate::games::eval_ml3::syntax::{EvalML3Env, EvalML3Expr, EvalML3Judgment, EvalML3Value};

    #[test]
    fn parses_fixture_040() {
        let source = include_str!("../../../copl/040.copl");
        let parsed = parse_source(source).expect("fixture should parse");
        assert_eq!(parsed.rule_name, "E-Fun");
        ...
        assert_eq!(
            ...
            EvalML3Judgment::EvalTo {
                ...
                expr: EvalML3Expr::Fun {
                    param: "x".to_string(),
                    body: Box::new(EvalML3Expr::BinOp {
                        op: crate::games::eval_ml3::syntax::EvalML3BinOp::Plus,
...
"#;
    ...
    fn parses_let_rec_expression_shape() {
        let source = "|- let rec f = fun x -> x in f 1 evalto 1 by E-Unknown {}";
        let parsed = parse_source(source).expect("parser should succeed");
        let EvalML3Judgment::EvalTo { expr, .. } = parsed.judgment else {
        ...
        assert_eq!(
            ...
            EvalML3Expr::LetRec {
                name: "f".to_string(),
                param: "x".to_string(),
                fun_body: Box::new(EvalML3Expr::Var("x".to_string())),
                body: Box::new(EvalML3Expr::App {
                    func: Box::new(EvalML3Expr::Var("f".to_string())),
                    arg: Box::new(EvalML3Expr::Int(1)),
                }),
            }
        );
    }
    ...
    fn parses_judgment_only_input_for_prover() {
        let parsed = parse_judgment_source("|- let rec f = fun x -> x + 1 in f 2 evalto 3")

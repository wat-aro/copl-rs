...
- Current game targets: `Nat`, `CompareNat1`, `CompareNat2`, `CompareNat3`, `EvalML1`, `EvalML1Err`, `EvalML2`, `EvalML3`, `EvalML4`, `EvalML5`, `EvalML6`, `EvalContML1`, `EvalContML4`, `EvalRefML3`, `TypingML2`, `TypingML3`, `TypingML4`, `TypingML5`, `TypingML6`, `PolyTypingML3`, `PolyTypingML4`, `NamelessML3`, `EvalNamelessML3`, `EvalNatExp`, `ReduceNatExp`, `While`.

### Out of scope now

- Prover implementation for games other than `Nat`, `CompareNat1`, `CompareNat2`, `CompareNat3`, `EvalML1`, `EvalML1Err`, `EvalML2`, `EvalML3`, `EvalML4`, `EvalML5`, `EvalML6`, `EvalContML1`, `EvalContML4`, `EvalRefML3`, `TypingML2`, `TypingML3`, `TypingML4`, `TypingML5`, `TypingML6`, `PolyTypingML3`, `PolyTypingML4`, `NamelessML3`, `EvalNamelessML3`, `EvalNatExp`, `ReduceNatExp`, and `While`.
...
- `src/games/eval_ml5/`:
  - `syntax.rs`, `lexer.rs`, `parser.rs`, `checker.rs`, `prover.rs`, `mod.rs` (`prove`).
- `src/games/eval_ml6/`:
  - `mod.rs` (`EvalML5`-backed checker/prover adapter for incremental bootstrap).
- `src/games/eval_cont_ml1/`:
  - `syntax.rs`, `lexer.rs`, `parser.rs`, `checker.rs`, `prover.rs`, `mod.rs` (`prove`).
- `src/games/eval_cont_ml4/`:
  - `syntax.rs`, `lexer.rs`, `parser.rs`, `checker.rs`, `prover.rs`, `mod.rs` (`prove`).
- `src/games/eval_ref_ml3/`:
  - `syntax.rs`, `lexer.rs`, `parser.rs`, `checker.rs`, `prover.rs`, `mod.rs` (`prove`).
- `src/games/typing_ml2/`:
  - `mod.rs` (`TypingML4`-backed checker/prover adapter for incremental bootstrap).
- `src/games/typing_ml3/`:
  - `mod.rs` (`TypingML4`-backed checker/prover adapter for incremental bootstrap).
- `src/games/typing_ml4/`:
  - `syntax.rs`, `lexer.rs`, `parser.rs`, `checker.rs`, `prover.rs`, `mod.rs` (`prove`).
- `src/games/typing_ml5/`:
  - `mod.rs` (`TypingML4`-backed checker/prover adapter for incremental bootstrap).
- `src/games/typing_ml6/`:
  - `mod.rs` (`TypingML4`-backed checker/prover adapter for incremental bootstrap).
- `src/games/poly_typing_ml3/`:
  - `mod.rs` (`PolyTypingML4`-backed checker/prover adapter for incremental bootstrap).
...
- `prover --game EvalML6` validates judgment-only input syntax before proving.
- `prover --game EvalContML1` validates judgment-only input syntax before proving.
- `prover --game EvalContML4` validates judgment-only input syntax before proving.
- `prover --game EvalRefML3` validates judgment-only input syntax before proving.
- `prover --game TypingML2` validates judgment-only input syntax before proving.
- `prover --game TypingML3` validates judgment-only input syntax before proving.
- `prover --game TypingML4` validates judgment-only input syntax before proving.
- `prover --game TypingML5` validates judgment-only input syntax before proving.
- `prover --game TypingML6` validates judgment-only input syntax before proving.
- `prover --game PolyTypingML3` validates judgment-only input syntax before proving.
- `prover --game PolyTypingML4` validates judgment-only input syntax before proving.
- `prover --game NamelessML3` validates judgment-only input syntax before proving.
- `prover --game EvalNamelessML3` validates judgment-only input syntax before proving.
- `prover --game EvalNatExp` validates judgment-only input syntax before proving.
- `prover --game ReduceNatExp` validates judgment-only input syntax before proving.
- `prover --game While` validates judgment-only input syntax before proving.
...
- TypingML5 prover route currently delegates to the TypingML4 prover core as an incremental bootstrap, preserving checker-compatible derivation output for the TypingML4-compatible typing-judgment subset.
- TypingML6 prover route currently delegates to the TypingML4 prover core as an incremental bootstrap, preserving checker-compatible derivation output for the TypingML4-compatible typing-judgment subset.
- PolyTypingML3 prover route currently delegates to the PolyTypingML4 prover core as an incremental bootstrap, preserving checker-compatible derivation output for the PolyTypingML4-compatible typing-judgment subset.
- PolyTypingML4 prover core builds an in-memory derivation AST using deterministic `T-*` rule construction with internal let-polymorphic type inference/unification for intermediate premise types.
- NamelessML3 prover core builds an in-memory derivation AST using deterministic `Tr-*` rule construction with nearest-binding de Bruijn index resolution (`Tr-Var1`, `Tr-Var2`).
- EvalNamelessML3 prover core builds an in-memory derivation AST using deterministic `E-*` / `B-*` rule construction with de Bruijn index lookup (`E-Var`) and recursive-closure application (`E-AppRec`).
- EvalNatExp prover core builds an in-memory derivation AST using deterministic recursive evaluation and Nat arithmetic rule construction (`E-*`, `P-*`, `T-*`).
- ReduceNatExp prover core builds an in-memory derivation AST using deterministic Nat arithmetic derivation construction (`P-*`, `T-*`) and reduction-relation derivation construction (`R-*`, `DR-*`, `MR-*`).
- While prover core builds an in-memory derivation AST using deterministic store-threading evaluation and rule-driven derivation construction (`A-*`, `B-*`, `C-*`) for command/store judgments.
-  provers render derivation trees in checker-compatible plain text and write them to stdout.
...
- For non-derivable Nat, CompareNat1, CompareNat2, CompareNat3, EvalML1, EvalML1Err, EvalML2, EvalML3, EvalML4, EvalML5, EvalML6, EvalContML1, EvalContML4, EvalRefML3, TypingML2, TypingML3, TypingML4, TypingML5, TypingML6, PolyTypingML3, PolyTypingML4, NamelessML3, EvalNamelessML3, EvalNatExp, ReduceNatExp, and While prover judgments, diagnostics are plain-text `RuleViolation` messages with `expected` / `actual` / `fix` where available.
...
Current EvalML6 checker/prover is enabled via an EvalML5-backed bootstrap path.

- `src/games/eval_ml6/mod.rs` adapts checker/prover routing to the EvalML5 implementation with `EvalML6` game-kind reporting.
- Judgment parsing/evaluation currently supports the EvalML5-compatible subset (`Gamma |- ... evalto ...`, arithmetic judgments, and EvalML5 pattern judgments).
- Successful check output remains the inferred root judgment text in checker-compatible plain text.
- Non-derivable prover judgments are reported as plain-text `RuleViolation` with actionable hints (`expected` / `actual` / `fix`) where derivation expectations are computable.
...
Current EvalContML4 checker/prover validates or generates derivation trees for CoPL ASCII input.
...
- `parser.rs` also parses judgment-only prover input (`Gamma |- ... evalto ...`, `... >> ... evalto ...`, `... => ... evalto ...`, `... plus ... is ...`, `... minus ... is ...`, `... times ... is ...`, `... less than ... is ...`).
- `syntax.rs` models EvalML4-style expressions/values extended with continuation values (`[k]`) and continuation-application judgments (`v => k evalto v'`), and includes `EvalContML4Derivation` rendering in checker-compatible textual form.
...
- `prover.rs` deterministically constructs EvalContML4 derivations by recursively evaluating expressions and continuations, and emitting matching `E-*` / `C-*` / `B-*` rule nodes.
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
...
Current TypingML4 checker/prover validates or generates derivation trees for CoPL ASCII input.
...
- `parser.rs` also parses judgment-only prover input (`Gamma |- e : t`).
- `syntax.rs` models TypingML4 expressions, types, type environments, and typing judgments (`Gamma |- e : t`).
- `syntax.rs` includes `TypingML4Derivation` rendering in checker-compatible textual form.
...
- `prover.rs` deterministically constructs TypingML4 derivations by recursive rule construction backed by internal monomorphic type inference/unification for intermediate premise types.
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
...
Current PolyTypingML3 checker/prover is enabled via a PolyTypingML4-backed bootstrap path.

- `src/games/poly_typing_ml3/mod.rs` adapts checker/prover routing to the PolyTypingML4 implementation with `PolyTypingML3` game-kind reporting.
- Checker and prover diagnostics rewrite game-name mentions from `PolyTypingML4` to `PolyTypingML3` so user-facing errors stay game-consistent.
- Successful check result text is the inferred root judgment (`Gamma |- e : t`).
- Non-derivable prover judgments are reported as plain-text `RuleViolation` with actionable hints (`expected` / `actual` / `fix`) where derivation expectations are computable.

Current PolyTypingML4 checker/prover validates or generates derivation trees for CoPL ASCII input.
...
- `syntax.rs` includes `PolyTypingML4Derivation` rendering in checker-compatible textual form.
...
- `prover.rs` deterministically constructs PolyTypingML4 derivations by recursive rule construction backed by internal let-polymorphic type inference/unification for intermediate premise types.
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
- Successful check result text is the inferred root judgment (`Gamma |- e : t`).
...
Current NamelessML3 checker/prover validates or generates derivation trees for CoPL ASCII input.
...
- `parser.rs` also parses judgment-only prover input (`Gamma |- e ==> e'`).
...
- `syntax.rs` includes `NamelessML3Derivation` rendering in checker-compatible textual form.
...
- `prover.rs` deterministically constructs translation derivations with recursive rule construction and nearest-binding de Bruijn index resolution.
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
...
Current EvalNamelessML3 checker/prover validates or generates derivation trees for CoPL ASCII input.
...
- `parser.rs` also parses judgment-only prover input (`Gamma |- e evalto v`, arithmetic/ordering builtin judgments).
- `syntax.rs` models nameless expressions (`let . =`, `fun . ->`, `let rec . = fun . ->`, application) and closure values (`(Gamma)[fun . -> e]`, `(Gamma)[rec . = fun . -> e]`).
- `syntax.rs` includes `EvalNamelessML3Derivation` rendering in checker-compatible textual form.
...
- `prover.rs` deterministically constructs evaluation derivations with recursive rule construction, de Bruijn index lookup, closure application (`E-App`), and recursive closure application (`E-AppRec`).
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
...
Current EvalNatExp checker/prover validates or generates derivation trees for CoPL ASCII input.

- `parser.rs` builds a generic derivation tree (`judgment + raw rule name + subderivations`).
- `parser.rs` also parses judgment-only prover input (`... evalto ...`, `... plus ... is ...`, `... times ... is ...`).
- `syntax.rs` models expressions (`n`, `e + e`, `e * e`), Nat terms, and judgments (`evalto`, `plus is`, `times is`).
- `syntax.rs` includes `EvalNatExpDerivation` rendering in checker-compatible textual form.
- `checker.rs` validates EvalNatExp and embedded Nat rules (`E-Const`, `E-Plus`, `E-Times`, `P-Zero`, `P-Succ`, `T-Zero`, `T-Succ`).
- `prover.rs` deterministically constructs EvalNatExp derivations by recursive expression evaluation and Nat arithmetic derivation construction (`P-*`, `T-*`).
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
...
Current ReduceNatExp checker/prover validates or generates derivation trees for CoPL ASCII input.

- `parser.rs` builds a generic derivation tree (`judgment + raw rule name + subderivations`).
- `parser.rs` also parses judgment-only prover input (`--->`, `-d->`, `-*->`, `plus`, `times`).
- `syntax.rs` models expressions (`n`, `e + e`, `e * e`), Nat terms, and judgments (`--->`, `-d->`, `-*->`, `plus is`, `times is`).
- `syntax.rs` includes `ReduceNatExpDerivation` rendering in checker-compatible textual form.
- `checker.rs` validates ReduceNatExp and embedded Nat rules (`R-*`, `DR-*`, `MR-*`, `P-Zero`, `P-Succ`, `T-Zero`, `T-Succ`).
- `prover.rs` constructs ReduceNatExp derivations by relation-specific proof construction (`R-*`, `DR-*`, `MR-*`) and Nat arithmetic derivation construction (`P-*`, `T-*`).
- Rule names are stored as raw text in the parsed tree and matched to static rule definitions in checker.
- Unknown rule names and premise arity mismatches are reported as `RuleViolation`.
- `RuleViolation` diagnostics carry the derivation node source location (`SourceSpan`), failing premise path (`root`, `1`, `1.2`, ...), and actionable hints where available.
...
- `prover --game TypingML5` currently accepts the TypingML4-compatible typing-judgment subset (`Gamma |- e : t`) and prints checker-compatible derivations via the TypingML4-backed bootstrap route.
- `prover --game TypingML6` currently accepts the TypingML4-compatible typing-judgment subset (`Gamma |- e : t`) and prints checker-compatible derivations via the TypingML4-backed bootstrap route.
- `prover --game PolyTypingML3` currently accepts the PolyTypingML4-compatible typing-judgment subset (`Gamma |- e : t`) and prints checker-compatible derivations via the PolyTypingML4-backed bootstrap route.
- `prover --game PolyTypingML4` parses judgment-only input (`Gamma |- e : t`), constructs derivation ASTs, and prints checker-compatible derivations.
- `prover --game NamelessML3` parses judgment-only input (`Gamma |- e ==> e'`), constructs derivation ASTs, and prints checker-compatible derivations.
- `prover --game EvalNamelessML3` parses judgment-only input (`Gamma |- e evalto v` / `plus` / `minus` / `times` / `less than`), constructs derivation ASTs, and prints checker-compatible derivations.
- `prover --game EvalNatExp` parses judgment-only input (`evalto` / `plus` / `times`), constructs derivation ASTs, and prints checker-compatible derivations.
- `prover --game ReduceNatExp` parses judgment-only input (`--->` / `-d->` / `-*->` / `plus` / `times`), constructs derivation ASTs, and prints checker-compatible derivations.
- `prover --game While` parses judgment-only input (`changes` / `evalto` for arithmetic/boolean store judgments), constructs derivation ASTs, and prints checker-compatible derivations.

Re-evaluation result (2026-02-19):

- Do not introduce a shared proof-search core at this point.
- Keep game-specific prover implementations (`Nat`, `CompareNat1`, `CompareNat2`, `CompareNat3`, `EvalML1`, `EvalML1Err`, `EvalML2`, `EvalML3`, `EvalML4`, `EvalML5`, `EvalML6`, `EvalContML1`, `EvalContML4`, `EvalRefML3`, `TypingML2`, `TypingML3`, `TypingML4`, `TypingML5`, `TypingML6`, `PolyTypingML3`, `PolyTypingML4`, `NamelessML3`, `EvalNamelessML3`, `EvalNatExp`, `ReduceNatExp`, `While`) as the default strategy.
- Rationale:
  - The twenty-six prover routes are deterministic, rule-driven evaluators/adapters, but their core domains differ (`Nat` arithmetic recursion, `CompareNat1` transitive less-than chaining, `CompareNat2` structural less-than recursion, `CompareNat3` right-side successor recursion, `EvalML1` expression evaluation, `EvalML1Err` error-aware evaluation, `EvalML2` environment-sensitive evaluation, `EvalML3` environment/closure-sensitive evaluation, `EvalML4` list/match-aware evaluation, `EvalML5` multi-clause/pattern-relation-aware evaluation, `EvalML6` EvalML5-backed bootstrap evaluation, `EvalContML1` continuation-machine evaluation, `EvalContML4` continuation-machine evaluation with environment/list/closure frames, `EvalRefML3` store-threading evaluation, `TypingML2` TypingML4-backed bootstrap typing evaluation, `TypingML3` TypingML4-backed bootstrap typing evaluation, `TypingML4` monomorphic typing-derivation construction, `TypingML5` TypingML4-backed bootstrap typing evaluation, `TypingML6` TypingML4-backed bootstrap typing evaluation, `PolyTypingML3` PolyTypingML4-backed bootstrap typing evaluation, `PolyTypingML4` let-polymorphic typing-derivation construction, `NamelessML3` translation-derivation construction, `EvalNamelessML3` nameless evaluation with de Bruijn closures/recursion, `EvalNatExp` Nat-expression evaluation, `ReduceNatExp` one-step/deterministic/multi-step reduction derivation construction, and `While` imperative-command/store evaluation with loop unfolding).
...
### 8.3 Unimplemented game onboarding status (2026-02-22)

- Official CoPL game pages currently have no remaining unimplemented game in this repository scope.
- `EvalRefML3` and `While` onboarding were completed with dedicated `syntax` / `lexer` / `parser` / `checker` / `prover` module sets.
...
- EvalML6 checker route is implemented as an EvalML5-backed adapter while preserving `GameKind::EvalML6` reporting.
- EvalML6 prover route is implemented as an EvalML5-backed adapter and rewrites game-name diagnostics from `EvalML5` to `EvalML6`.
...
- TypingML3 prover route is implemented as a TypingML4-backed adapter and rewrites game-name diagnostics from `TypingML4` to `TypingML3`.
- TypingML5 checker route is implemented as a TypingML4-backed adapter while preserving `GameKind::TypingML5` reporting.
- TypingML5 prover route is implemented as a TypingML4-backed adapter and rewrites game-name diagnostics from `TypingML4` to `TypingML5`.
- TypingML6 checker route is implemented as a TypingML4-backed adapter while preserving `GameKind::TypingML6` reporting.
- TypingML6 prover route is implemented as a TypingML4-backed adapter and rewrites game-name diagnostics from `TypingML4` to `TypingML6`.
...
- TypingML4 prover is implemented with game-specific recursive typing derivation construction that deterministically emits `T-*` derivations and checker-compatible pretty-printed output.
- PolyTypingML3 checker route is implemented as a PolyTypingML4-backed adapter while preserving `GameKind::PolyTypingML3` reporting.
- PolyTypingML3 prover route is implemented as a PolyTypingML4-backed adapter and rewrites game-name diagnostics from `PolyTypingML4` to `PolyTypingML3`.
...
- PolyTypingML4 prover is implemented with game-specific recursive typing derivation construction that deterministically emits `T-*` derivations and checker-compatible pretty-printed output.
...
- NamelessML3 prover is implemented with game-specific recursive translation derivation construction that deterministically emits `Tr-*` derivations and checker-compatible pretty-printed output.
...
- EvalNamelessML3 prover is implemented with game-specific recursive evaluation derivation construction that deterministically emits `E-*` / `B-*` derivations and checker-compatible pretty-printed output.
...
- EvalNatExp prover is implemented with game-specific recursive evaluation/arith-derivation construction that deterministically emits `E-*` / `P-*` / `T-*` derivations and checker-compatible pretty-printed output.
...
- ReduceNatExp prover is implemented with game-specific recursive reduction/arith-derivation construction that deterministically emits `R-*` / `DR-*` / `MR-*` / `P-*` / `T-*` derivations and checker-compatible pretty-printed output.
...
- While checker is implemented with the same parser/checker boundary policy as Nat (raw rule names in parser, rule resolution in checker), validating `A-*` / `B-*` / `C-*` rule applications for store-threading judgments (`sigma |- a evalto i`, `sigma |- b evalto bool`, `c changes sigma1 to sigma2`) with `SourceSpan` + `premise path` diagnostics.
- While prover is implemented with deterministic store-threading evaluation that emits `A-*` / `B-*` / `C-*` derivations and checker-compatible pretty-printed output.

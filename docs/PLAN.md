...
- ユーザーが特定タスクのコミットを明示要求した場合は、そのタスクをコミット境界として扱う。
- ユーザーがコミットを要求したタスクは、コミット作成完了まで未完了として扱う（`[x]` を付けた時点では完了扱いにしない）。
- 当該タスクのコミット前検証（`cargo fmt` / `cargo test` / `cargo clippy --all-targets --all-features -- -D warnings`）完了後は、次タスクへ進む前に即時コミットする。
- コミット準備中は他タスクの新規編集を入れない。検証のために一時退避・一時復元を行う場合も、コミット後に事前状態へ戻すだけに限定する。
...
- 運用ルール:
  ...
  - ユーザーが特定タスクのコミットを明示要求した場合、当該タスクはコミット作成完了まで未完了として扱う。
  - ユーザーが特定タスクのコミットを明示要求した場合、コミット前検証完了後は次タスク着手前に即時コミットする。
